...
CREATE USER admin@'%' IDENTIFIED WITH mysql_native_password BY '_admin_pwd';
CREATE USER admin@'localhost' IDENTIFIED WITH mysql_native_password BY '_admin_pwd';

CREATE DATABASE IF NOT EXISTS alertflex;
CREATE DATABASE IF NOT EXISTS afevents;

GRANT ALL ON alertflex.* TO admin@'%';
GRANT ALL ON alertflex.* TO admin@'localhost';
GRANT ALL ON afevents.* TO admin@'%';
